...
// pageNumber.innerHTML=`you in ${} page`;
...
let mainFlexBox = document.getElementById("mainFlexContainer");
//create  1st div
let button1 = document.createElement("button");
button1.setAttribute("id", "first");
button1.innerHTML = "First";
mainFlexBox.append(button1);
...
//adding button to mainflexbox w. r. to data.length
for (let i = 1; i <= data.length; i++) {
  //create  a other divs
  //adding this to mainflexbox 2,3,4,5,6

  if (i > 1 && i < 7) {
    let button = document.createElement("button");
    button.innerHTML = i; //2,3,4,5,6
    button.setAttribute("id", i);
    mainFlexBox.append(button);
    document.getElementById(i).addEventListener("click", (e) => {
      console.log(e.target.innerHTML);
      //first i=0,(data[0], id=1),i=2 data[2-1]=data[1],i=3 data[3-1]=data[2]
      pageNumber.innerHTML = JSON.stringify(data[i - 1]);
      nextButton.disabled = false;
...
mainFlexBox.append(nextButton);

//create last button
let lastButton = document.createElement("button");
lastButton.innerHTML = "Last";
lastButton.setAttribute("id", "last");
mainFlexBox.append(lastButton);
//create previous button
let button2 = document.createElement("button");
button2.setAttribute("id", "Previous");
button2.innerHTML = "Previous";
mainFlexBox.append(button2);

document.getElementById("next").addEventListener("click", (e) => {
  console.log(pageNumber.innerHTML);
  if (!pageNumber.innerHTML) {
    pageNumber.innerHTML = JSON.stringify(data[0]); //1st element
  } else {
    if (JSON.parse(pageNumber.innerHTML).id <= data.length - 1) {
      console.log(JSON.parse(pageNumber.innerHTML).id);
      pageNumber.innerHTML = JSON.stringify(
        data[JSON.parse(pageNumber.innerHTML).id]
      ); //1st element
      console.log(JSON.stringify(data[JSON.parse(pageNumber.innerHTML).id]));
    } else {
...
document.getElementById("last").addEventListener("click", (e) => {
  ...
  pageNumber.innerHTML = JSON.stringify(data[data.length - 1]);
  nextButton.disabled = true;
  lastButton.disabled = true;
  button2.disabled = false;
});

//click event for 1st button
document.getElementById("first").addEventListener("click", (e) => {
  console.log(e.target.innerHTML);

  pageNumber.innerHTML = JSON.stringify(data[0]);
  nextButton.disabled = false;
  lastButton.disabled = false;
  button2.disabled = true;
});
...
document.getElementById("Previous").addEventListener("click", (e) => {
  console.log(
    ...
    JSON.parse(pageNumber.innerHTML).id - 1
  );
  if (JSON.parse(pageNumber.innerHTML).id - 1 > 0) {
    // console.log("pageNumber.innerHTML", pageNumber.innerHTML);
    pageNumber.innerHTML = JSON.stringify(
      data[JSON.parse(pageNumber.innerHTML).id - 2]
    );
    nextButton.disabled = false;
    lastButton.disabled = false;
    button2.disabled = false;
    // console.log(data[((JSON.parse(pageNumber.innerHTML).id)-2)]);
  } else {
    nextButton.disabled = false;
    lastButton.disabled = false;
    button2.disabled = true;
